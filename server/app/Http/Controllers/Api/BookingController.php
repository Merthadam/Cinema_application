    ...
    public function store(Request $request)
    {
        try {
            ...
            foreach ($request->seats as $seat) {
                if ($seat['row'] > $room->rows || $seat['seat'] > $room->seats_per_row) {
                    return response()->json([
                        'message' => "Invalid seat position: row {$seat['row']}, number {$seat['seat']}"
                    ], 422);
                }
            }
            ...
            foreach ($request->seats as $requestedSeat) {
                foreach ($existingBookings as $booking) {
                    ...
                    foreach ($bookedSeats as $bookedSeat) {
                        if (!isset($bookedSeat['row']) || !isset($bookedSeat['seat'])) continue;
                        
                        if ($bookedSeat['row'] == $requestedSeat['row'] && 
                            $bookedSeat['seat'] == $requestedSeat['seat']) {
                            return response()->json([
                                'message' => "Seat row {$requestedSeat['row']}, number {$requestedSeat['seat']} is already taken"
                            ], 422);
                        }
                    }
                }
            ...
            $booking = Booking::create([
                ...
                'seats' => json_encode($formattedSeats),
